...
/world/Topic(T, addr, master, key)
	...
	if("ping" in input)
		...
		var/list/set_names = list(
			"heads" = omegacorp_positions,
			"sec" = guards_positions,
			"eng" = villagers_positions,
			"med" = religious_positions,
			"sci" = wizards_positions,
			"car" = nobility_positions,
...
/world/Reboot(var/reason, var/feedback_c, var/feedback_r, var/time)
	...
	spawn(0)
		world << sound(pick('sound/misc/fleshwound.ogg','sound/misc/thatwaseasy.ogg', 'sound/goonstation/misc/newround1.ogg'))// random end sounds!! - LastyBatsy
